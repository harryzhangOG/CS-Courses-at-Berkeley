...
(deep-map (lambda (x) (* x x)) '(1 (2 (3))))
; expect (1 (4 (9)))
(define (concat a b) (cond ((null? a) b)
  (else (cons (car a) (concat (cdr a) b)))))
; expect concat
(concat '(1 2 3) '(4 5 6))
; expect (1 2 3 4 5 6)
(define (reverse lst) (define (reverse-sofar lst lst-sofar)
...
; expect (5 4 3 2 1)
(define (pairs L)
(define (accum lst result)
(cond ((null? lst) result)
((or (null? (cdr lst)) (null? (cdr (cdr lst))))
(cons lst result))
(else (accum (cdr (cdr lst))
(cons (list (car lst) (car (cdr lst))) result)))))
(reverse (accum L '())))
(pairs '(1 2 3 4))
; expect ((1 2) (3 4))
(pairs '())
...
(define (f s t) (cond ((pair? s) (+ t
  (f (car s) 2) (f (cdr s) 0))) ((null? s) t)
  (else 0)))
(parens (list 3 3))
; expect 2
(parens '((3) ((3))))
; expect 8
(define (contains num lst) (cond
  ((null? lst) False)
  ((= num (car lst)) True)
  (else (contains num (cdr lst)))))
(contains 3 '(1 2 3 4))
; expect #t
(contains 8 '(3 4 1))
; expect #f
