...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(val)
        #E C 21
        if isinstance(val, MacroProcedure): #?????
            return scheme_eval(val.apply_macro(rest,env), env)
        rest_val = rest.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(val, rest_val, env)
        #used to be scheme_apply, modified for p21
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    while temp.second is not nil:
        ...
        temp=temp.second
    return scheme_eval(temp.first, env, True) #modified for Problem 20
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    return result #modified for extra credit
    #if expressions == nil:
    #    return True
    #else:
...
def do_define_macro(expressions, env):
    ...
    #rocedure = MacroProcedure(formals, body, env)
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        return LambdaProcedure.make_call_frame(self, args, env)
    # END PROBLEM 16
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        #while not scheme_symbolp(result):
