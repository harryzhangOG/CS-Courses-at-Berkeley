...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if who==0:
            return announce_highest(who,max(previous_high,gain),score0)
        else:
            return announce_highest(who,max(previous_high,gain),score1)
    return say
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    maxAve=0
    maxNum=0
    makeAve=make_averaged(roll_dice)
    k=10
    while k>0:
        ave=makeAve(k,dice)
        if ave>maxAve:
...
the first part is to by making a trap which will trick opponent into rolling
if it can produce a beneficial swap
the second part is similar to bacon strategy
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    def trap(score,opponent_score):
        trapRoll=0
        k=1
        while k<11:
            trapRoll=k
            if is_swap(score,opponent_score+k) and opponent_score>score:
                return trapRoll
            k=k+1
        return 1
    baconScore=free_bacon(opponent_score)
    temp=trap(score,opponent_score)
    diff=abs(score-opponent_score)
    if baconScore>diff and score<opponent_score:
        return 0
    if temp>1:
        return trap(score,opponent_score)
    if (score+baconScore)>100:
        return 0
