        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if insect.can_contain(self.ant):
                    ...
                    assert self.ant is None, 'Two ants in {0}'.format(self) #modified when doing problem 10
                # END Problem 9
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        increment=0
        place=self.place
        #while place.entrance!=hive and len(place.bees)==0:
        #    place=place.entrance
        #    increment+=1
        #    if increment in range(self.min_range,self.max_range+1) and increment!=4:

        #        return random_or_none(place.bees)
        #return None
        while place.entrance!=None or place!=hive:
            if place.bees and increment >= self.min_range and increment <= self.max_range:
                return random_or_none(place.bees)
            else:
                place=place.entrance
                increment+=1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def double_damage(loc):
            ant=loc.ant
            if not isinstance(ant,QueenAnt):
                if ant!=None:
                    if ant not in self.behind:
                        self.behind.append(ant)
                        ant.damage*=2
                    if ant.container and ant.ant!=None:
                        inside=ant.ant
                        if not isinstance(inside,QueenAnt):
                            if inside not in self.behind:
                                self.behind.append(inside)
                                inside.damage*=2
        while next_place.exit!=None:
            double_damage(next_place)
