...
class BoardWidget extends Pad {
    ...
    /** Draw spear from G and S. */
    private void drawSpear(Graphics2D g) {
        if (_spears != null) {
            for (Square spear : _spears) {
                g.drawImage(_spear, cx(spear.col()) + 2,
                        cy(spear.row()) + 4, null);
            }
        }
    }
    ...
    private void click(Square s) {
        ...
        if (_moves.size() == 3) {
            String cmnd = "";
            for (int i = 0; i < _moves.size(); i += 1) {
                cmnd += _moves.get(i) + " ";
                repaint();
            }
            _moves.clear();
            _commands.offer(cmnd);
        }
        update(_board);
        repaint();
    }
    ...
    /** Find queen from BOARD. */
    public void findQueen(Board board) {
        ArrayList<Square> white = new ArrayList<>();
        ArrayList<Square> black = new ArrayList<>();
        for (int x = 0; x < Board.SIZE; x += 1) {
            for (int y = 0; y < Board.SIZE; y++) {
                if (board.get(Square.sq(x, y)).equals(Piece.WHITE)) {
                    white.add(Square.sq(x, y));
                }
                if (board.get(Square.sq(x, y)).equals(Piece.BLACK)) {
    ...
    public void findSpears(Board board) {
        ArrayList<Square> spear = new ArrayList<>();
        for (int x = 0; x < Board.SIZE; x++) {
            for (int y = 0; y < Board.SIZE; y++) {
                if (board.get(Square.sq(x, y)).equals(Piece.SPEAR)) {
