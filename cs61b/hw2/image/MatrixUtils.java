...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
       double [][] copym = copy(m);
        for (int r = 1; r < m.length; r +=1) {
            for (int c = 0; c < m[0].length; c+=1) {
                double min = Double.POSITIVE_INFINITY;
                for (int d = -1; d <= 1; d += 1) {
                    double temp = get(copym, r - 1, c + d);
                    if (temp < min) {
                        min = temp;
                    }
                }
    ...
    public static double[][] transpose(double[][] m) {
        int row = m.length;
        int col = m[0].length;
        double[][] transp = new double[col][row];
     ...
     */
    /** helper function that returns the index of the smallest entry
     * in a 2d array
     * @param d
     * @return index of smallest entry of d
     */
    ...
    public static int[] findVerticalSeam(double[][] m) {
        ...
        vertical[row - 1] = helper(m[row - 1]);
        for (int r = row - 2; r >= 0; r -= 1) {
            int low = vertical[r + 1] - 1;
            int high = vertical[r+1] +1;
            if (low < 0) {
                low = 0;
            }
            if (high >= m[r].length) {
